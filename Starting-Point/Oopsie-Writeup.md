...
Start BurpSuite and navigate to the Proxy tab. From there, select "Open browser." Enter the URL for the Web Server (http://<target_IP>) and then navigate to the "Target" tab in BurpSuite. We can see a GET request for **/cdn-cgi/login/**
...
We are provided with a page that has an access ID of **34322** and the name listed as **admin**. We then move back to our developer console and see if we can manipulate the cookies.
...
Now that we have access to the Uploads page, we can see that it allows file uploads. We will attempt to upload a PHP reverse shell.

If you are on Kali, install webshells:

```sudo apt update && sudo apt install webshells -y```

Start a netcat listener in a **separate** terminal session. This will be useful for establishing a reverse shell.
...
Change the **$ip** field to your tun0 IP address and the **$port** to 1337. Press ctrl + o and then hit enter to save the file. Now, go to the uploads section of the target webpage and upload the file. 
...
gobuster finds the **/uploads** directory. We don't have permission to access the directory, but we can try to access our uploaded php file.
...
Upon inspection, we can see a directory called ```/var/www/html/cdn-cgi/login```. Head to the login directory.

**Note:** In a more secure environment, we would search for keywords like ```"*conf"``` ```"*settings"``` and ```"*.env"```
...
This file lists usernames. Notice that bug-reporting says "admin" next to it. This is useful information worth noting.
...
You will see that we are root. Navigate to the **/root** directory
